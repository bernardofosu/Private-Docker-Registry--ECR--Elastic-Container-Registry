...
docker stop 0fc92c48

docker ps    # running containers
docker ps -a # includes stopped ones

#If you forgot the ID, find the container using port 8501
docker ps --filter "publish=8501"
```

Next time, give it a name so it’s easier to stop

Run:
```sh
docker run -d --name streamlit-app -p 8501:8501 440744234244.dkr.ecr.us-east-1.amazonaws.com/privatedockerregistry:v1
```
Then stop with:
```sh
...
Check logs if it won’t stop
```sh
docker logs -f 0fc92c4853bb
```
